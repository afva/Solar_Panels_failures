...
class CNN:
    ...
    def _add_output_layers(self, class_count: int, fc_layer_size: int = 1024):
        ...
        # Add new layers
        model.add(tf.keras.layers.Dense(fc_layer_size*2, activation='sigmoid', kernel_regularizer=L2(0.05)))
        model.add(tf.keras.layers.Dropout(0.5))
        model.add(tf.keras.layers.Dense(fc_layer_size*2, activation='relu', kernel_regularizer=L2(0.05)))
        model.add(tf.keras.layers.Dropout(0.5))
        model.add(tf.keras.layers.Dense(int(1.5*fc_layer_size), activation='relu', kernel_regularizer=L2(0.05)))
